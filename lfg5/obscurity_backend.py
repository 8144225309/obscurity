...
class DataManager:
    ...
    def ensure_directories(self):
        ...
        for d in [DATA_DIR, CHAINS_DIR, KEYSTORE_DIR]:
            ...
            # Create a README for keystore
            if d == KEYSTORE_DIR and not os.path.exists(os.path.join(d, "README.txt")):
                 with open(os.path.join(d, "README.txt"), "w") as f:
                    f.write("Obscurity Key Storage.\nWARNING: Handle with care.\n")

        if not os.path.exists(CONFIG_FILE):
            # Default to standard Bitcoin Core paths
            default_data = os.path.join(os.getenv('APPDATA'), "Bitcoin") if os.name == 'nt' else os.path.expanduser("~/.bitcoin")
            
            default_config = {
                "rpc_host": "127.0.0.1",
                "rpc_port": 8332,
                "rpc_user": "",     # Leave empty to use cookie
                "rpc_pass": "",     # Leave empty to use cookie
    ...
    def test_rpc_connection(self):
        """Simple ping to see if we can talk to the node."""
    ...
    def verify_on_chain(self, txid, expected_hash):
        """Fetches Tx, extracts P2PK outputs, checks for block hash."""
        ...
        for vout in tx.get('vout', []):
            ...
            # Simple heuristic for P2PK (21...ac)
            if len(hex_spk) == 70 and hex_spk.startswith("21") and hex_spk.endswith("ac"):
                pubkey = hex_spk[2:68]
                payload_chunk = pubkey[2:10] # Simulated extraction
