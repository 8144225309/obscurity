...
class ObscurityApp(ctk.CTk):
    def __init__(self):
        ...
        # Window Setup
        self.title("Obscurity [Fork-Aware] // Hackathon Build v0.4")
    ...
    def create_config_view(self, parent):
        ...
        # ZMQ (Greyed Out)
        tk.Label(form, text="--- ZMQ Settings (Scanning) ---", bg="#2b2b2b", fg="gray").grid(row=4, column=1, pady=10)
        
        ctk.CTkLabel(form, text="ZMQ Host:", text_color="gray").grid(row=5, column=0, sticky="e", padx=10)
        e_zmq_h = ctk.CTkEntry(form, width=300, text_color="gray")
        e_zmq_h.insert(0, "127.0.0.1")
        e_zmq_h.configure(state="disabled") # Greyed out
        e_zmq_h.grid(row=5, column=1, padx=10, pady=5)

        ctk.CTkLabel(form, text="ZMQ Port:", text_color="gray").grid(row=6, column=0, sticky="e", padx=10)
        e_zmq_p = ctk.CTkEntry(form, width=300, text_color="gray")
        e_zmq_p.insert(0, "28332")
        e_zmq_p.configure(state="disabled") # Greyed out
        e_zmq_p.grid(row=6, column=1, padx=10, pady=5)

        ctk.CTkButton(frame, text="Save Config", width=200, height=40, fg_color="#1f538d", command=self.action_save_config).pack(pady=30)
        return frame
    ...
    def action_verify(self):
        ...
        txid = self.ent_txid.get().strip()
        if not txid: 
             self.txt_ver_log.insert("end", "Error: No TxID linked.\n"); return
        ...
        # Load block to get expected hash
        # (Short cut: just re-read file)
        # We need the full hash, so finding it via data manager
        # But for demo we just pass what we know or get it from file
        # Re-load:
        block_data = self.data_manager.update_block_status(self.current_chain, idx) # just loads
    ...
    def on_block_select(self, event):
        ...
        # Could auto-fill inputs here too
